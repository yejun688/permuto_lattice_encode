...
class ReconDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        if self.use_depth or self.use_bubble:
            ...
            for scale_mat, depth_path, intrinsics, pose, i in tzip(scale_mats, depth_paths, self.intrinsics_all, self.pose_all, range(len(self.pose_all))):
                ...
                # 处理深度图像，将深度图转换为点云，特别是在深度遮罩（depth_mask）的情况下，每个图像像素被投影到三维空间，形成点云。
                if self.use_bubble:
                    # 创建大小为self.total_pixels 的张量，初始值为-1，这个张量用于存储每个像素对应的点云索引，初始值为 -1 表示没有链接
                    pointlink = -torch.ones([self.total_pixels], dtype=torch.long)
                    # 通过深度遮罩 depth_mask 筛选有效的像素。depth_mask 是一个布尔掩码，指示哪些像素在深度图中是有效的
                    pointlink[depth_mask] = torch.arange(0, len(valid_indices), dtype=torch.long) + n_points
                    # 生成一个像素超链接 pixlink ,它是当前图像中有效像素的索引
                    ...
                    #  将有效像素的深度信息转换为三维空间中的点。该函数通常会基于相机的内参（intrinsics）和位姿（pose）将深度图中的像素转换为真实的世界坐标系中的点。
            ...
            if self.use_bubble:
                # 将之前的生成的多个点云（存储在 self.pointcloud 列表中）沿第0维度拼接起来，形成一个大的点云张量
                self.pointcloud = torch.cat(self.pointcloud, 0)
                # pointlinks是像素和点云索引之间的映射
    ...
    # 这段代码定义了一个collate_fn函数,它用来处理batch_list,通常是数据加载器中传递的一小批次数据，并将其转换为适合输入模型训练的数据结构，collate_fn 通常用于将一个批次的数据聚合成模型可以接受的形式。
